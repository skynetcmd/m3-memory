...
- **Tool catalog**: `bin/mcp_tool_catalog.py` is the single source of truth for all MCP tool definitions via the `ToolSpec` dataclass. 44 tools total (35 default-allowed + 9 destructive opt-in).
...
- **Sources**: Composes its tool list from three places — `PROTOCOL_TOOLS` (5 inline), `DEBUG_TOOLS` (6 inline), and `bin/mcp_tool_catalog.py` (35 default / 44 with destructive enabled).
