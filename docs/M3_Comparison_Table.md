...
**M3 standing — QA accuracy:** **92.0%** with **no oracle metadata** *(SHR=100% at k=20; QA is very model-dependent)* — the end-to-end figure, scored by the upstream gpt-4o judge, with all routing inferred from the question text at runtime (the earlier oracle-routed configuration scored 89.0%). Because it rises and falls with whatever answer model reads the retrieved context, compare it only against other systems' QA-accuracy numbers, never against their recall figures. *(Methodology: the specific answer model used for the 92.0% run is recorded in the [LME-S Benchmarking Report](../benchmarks/longmemeval/LME-S_Benchmarking_Report.md).)*
