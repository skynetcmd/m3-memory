...
Some agents can't speak MCP directly — Aider and Openclaw are the two we support today. For them, route chat completions through the bundled [`mcp_proxy`](../bin/mcp_proxy.py), an OpenAI-compatible server on `localhost:9000` that injects m3-memory tools (and the rest of the catalog — 96 tools total) into every request and executes `tool_calls` by calling bridge functions directly.
...
- [AGENT_INSTRUCTIONS.md](./AGENT_INSTRUCTIONS.md) — all 73 MCP tools and agent behavioral rules
