...
| `M3_TITLE_MATCH_BOOST` | `0.15` | Multiplier for the title-overlap boost: if a fraction `f` of query tokens appear in the title, add `M3_TITLE_MATCH_BOOST * f` to the final score. Set to `0` to disable. |
| `M3_IMPORTANCE_WEIGHT` | `0.15` | Weight of the caller-supplied `importance` field (0.0–1.0) in final ranking. Set to `0` to ignore importance entirely. |
...
| `M3_SEARCH_ROW_CAP` | `5000` | Cap on the candidate rows carried into cosine scoring. It trims the set **after** the SQL fetch, which is itself limited to 2,000 rows on the default path — so this is a backstop against a misconfigured limit, not the active bound. Raising it will not widen a search. *(legacy alias: `SEARCH_ROW_CAP`, still honored via `getenv_compat`.)* |
...
| `M3_QUERY_TYPE_ROUTING` | `1` (**on by default**) | Retrieval-side: when a query matches "When/what date/which day" plus a proper noun, shift `vector_weight` to `0.3` (BM25-heavy) so the named-entity signal isn't diluted by embedding similarity. |
...
| `M3_FACT_ENRICH_MAX_ATTEMPTS` | `3` | Maximum retries for failed enrichment queue items before they are marked as poison (poisoned items remain visible in queue with `last_error` for manual inspection). |
...
| `M3_ENTITY_RESOLVE_COSINE_MIN` | `0.92` | Minimum embedding cosine similarity for cosine-match resolution tier (final fallback before creating a new entity). |
