...
This document provides a comprehensive inventory of all 116 MCP tools available in the M3 Memory system.

> **How this reconciles with other docs.** The **116** in-catalog tools (the number the README/COMPARISON quote as "100+", and the `count` in `docs/tools/MCP_CATALOG.json`, which excludes the two `tools_*` meta-tools → **114** there) plus **0** MCP-proxy protocol/debug tools (`mcp_proxy.py`) = **116** total listed here. The prose "100+ tools" refers to the in-catalog surface; this inventory additionally documents the proxy transport tools.
