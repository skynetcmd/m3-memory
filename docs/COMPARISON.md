...
| **MCP support** | Native — 73 tools, zero config in Claude Code / Gemini CLI | No native MCP; requires a custom wrapper |
...
M3-Memory is a **dedicated, lightweight memory layer** — a drop-in backend for agents you already have. It does one thing: give your agent persistent, private, consistent memory via 73 MCP tools.
...
| **MCP support** | Native — 73 tools, zero config | Custom SDKs / REST API |
...
| **MCP support** | Native — 73 tools | No |
