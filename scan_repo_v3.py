...
SCANNERS = [
    # Core Scanners (Security)
    ('gitleaks',  ['gitleaks', 'detect', '--source', '{repo}', '--report-format', 'sarif', '--report-path', '{out}/gitleaks.sarif', '--no-banner', '--exit-code', '0'], 'gitleaks.sarif', 'SARIF'),
    ...
    # Auxiliary Scanners (Extended Security)
    ...
    # Quality Scanners (Lint & Static Analysis)
    ('ruff',      ['ruff', 'check', '--format', 'sarif', '--output-file', '{out}/ruff.sarif', '{repo}'], 'ruff.sarif', 'SARIF'),
    ('mypy',      ['mypy', '{repo}', '--ignore-missing-imports', '--explicit-package-bases'], 'mypy.txt', None), # Mypy stdout used for report
]
