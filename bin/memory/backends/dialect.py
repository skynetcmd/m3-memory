...
class Dialect:
    ...
    def claim_message(
        ...
        work that follows. SQLite has a single write lock for the WHOLE database,
        so a caller that claims and then does its work inside the transaction
        serializes every other writer behind it. Measured on this box, 8
        processes claiming with only 10ms of work held inside the txn: p50 10.6ms
        ...
        ``FOR UPDATE SKIP LOCKED`` SKIPS a contended row, while SQLite BLOCKS the
