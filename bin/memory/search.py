...
def _resolve_search_callable(name: str):
    ...
    return globals()[name]
from .fts import (
    _DATE_RE_ISO,  # noqa: F401 — memory_core lazy-registry shim resolves this from memory.search
...
# ──────────────────────────────────────────────────────────────────────────────
# Re-imports: pure ranking + routing helpers extracted to memory.search_ranking
# / memory.search_routing. Re-imported here (rather than left to only live in
# the new modules) so `memory.search.<name>` keeps resolving for direct
# importers AND so memory_core's lazy-registry shim (`_LAZY_IMPORTS[name] =
# "memory.search"`) — which does `getattr(importlib.import_module("memory.
# search"), name)` — continues to find these names as attributes of this
# module. Do NOT remove even though this module no longer calls them itself.
# ──────────────────────────────────────────────────────────────────────────────
from .search_ranking import (  # noqa: F401 — re-exported for callers + memory_core shim
    _DATE_MONTHS,
...
from .search_routing import (  # noqa: F401 — re-exported for callers + memory_core shim
    _apply_auto_layer,
    _apply_sharp_trim,
    _detect_sqlite_vec,
...
# NOTE: _pull_predecessor_turns, _maybe_route_query, _apply_recency_bonus,
# _trim_by_elbow, _apply_temporal_boost, and _DATE_MONTHS moved to
# memory.search_ranking / memory.search_routing (pure helpers — see module
# docstrings there). Re-imported below so `memory.search.<name>` and the
# memory_core lazy-registry shim keep resolving to the same function objects.
...
# strict no-op.
#
# NOTE: _UNSET stays here (it's a module-level sentinel referenced by the
# memory_core shim, but not used by any function that moved out).
# _extract_caller_overrides, _apply_auto_layer, _apply_sharp_trim,
# is_temporal_query, _detect_sqlite_vec, _hybrid_score_batch, and
# _recency_bonus_ranks moved to memory.search_ranking / memory.search_routing
# — see the re-import block near the top of this module.
# ---------------------------------------------------------------------------
