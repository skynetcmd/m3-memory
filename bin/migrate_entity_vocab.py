#!/usr/bin/env python3
"""One-shot migration: rename v1 entity vocabulary to v2.

Performs in-place rename of entity_type and predicate values in any DB that
was extracted under the v1 vocabulary (default before 2026-05-03). Idempotent
-- re-running has no effect on already-migrated rows since the v1 names no
longer exist after the first pass.

Renames performed:

  entities.entity_type:
    'concept'   -> 'legacy_concept'   (preserved-but-deprecated)
    'object'    -> 'legacy_object'    (preserved-but-deprecated)

  entity_relationships.predicate:
    'relates_to' -> 'mentions'    (semantic substitution: both catch-all references)
    'contradicts' -> 'supersedes' (semantic substitution: change-edge canonical)

Left as-is (still valid in v2 schema as deprecated):
    'before', 'after'  predicate rows -- temporal ordering now derived from
        has_time edges, but old rows remain queryable.

Backward-compat: not maintained. v1 type names ('concept', 'object') and v1
predicate names ('relates_to', 'contradicts') are NOT in the v2 vocab; new
extractors must use v2 names. Existing v1 rows would fail validation on
re-write until this script renames them.
