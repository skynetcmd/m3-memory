...
AfterModel (per-turn) is on full parity with Claude's Stop hook — scream on fail.

Handles: AfterModel (per-turn), PreCompress, SessionEnd
...
def main() -> int:
    ...
    reason_field = envelope.get("reason", "")  # Gemini-specific: exit reason

    if not transcript:
        print(f"{AGENT} hook: envelope missing transcript_path", file=sys.stderr)
        return 1

    # Gemini-specific variant mapping
    variant_map = {
