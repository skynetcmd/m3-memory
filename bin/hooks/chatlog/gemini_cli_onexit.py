#!/usr/bin/env python3
"""Gemini CLI AfterModel / PreCompress / SessionEnd hook -> m3-memory chatlog ingest.

Cross-platform (Windows, macOS, Linux). Reads the Gemini CLI hook envelope
...
All events get full scream-on-fail — AfterModel (per-turn) is on parity with
Claude's Stop hook.

Envelope (stdin JSON):
  { "session_id": "...", "transcript_path": "...",
    "cwd": "...", "hook_event_name": "AfterModel" | "PreCompress" | "SessionEnd",
    "reason": "exit" | "clear" | ... }
"""
import json
import os
...
def write_fallback(transcript: str, session_id: str, event: str,
    ...
    fallback = save_dir / f"m3_unsaved_gemini_{sid}_{ts}.md"
...
def main() -> int:
    ...
    if not raw:
        print("gemini_cli_onexit: empty stdin envelope", file=sys.stderr)
        return 1

    try:
        envelope = json.loads(raw)
    except json.JSONDecodeError as e:
        print(f"gemini_cli_onexit: malformed JSON envelope: {e}", file=sys.stderr)
        return 1

    transcript = envelope.get("transcript_path", "")
    session_id = envelope.get("session_id", "")
    event_name = envelope.get("hook_event_name", "")
    reason_field = envelope.get("reason", "")

    if not transcript:
        print("gemini_cli_onexit: envelope missing transcript_path", file=sys.stderr)
        return 1

    variant_map = {
        "AfterModel":   "after_model",
        "PreCompress":  "pre_compress",
        "SessionEnd":   f"session_end_{reason_field}" if reason_field else "session_end",
    }
    variant = variant_map.get(event_name, "gemini_cli")
    ...
    py = find_python(repo)
    cmd = [py, str(ingest), "--format", "gemini-cli",
