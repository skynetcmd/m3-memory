...
Local-first Agentic Memory Layer Framework for MCP Agents • 103 tools • Hybrid search (FTS5 + vector + MMR) • Directory ingestion & file-memory • GDPR • FIPS 140-3 ready • 100% local
...
## 🎚️ 103 tools, but they don't all crowd your context — domain gating keeps the catalog small

M3 exposes 103 MCP tools so power users can customize at fine granularity —
...
eager catalog while giving the agent the full 103 tools whenever it
actually needs them.
...
**Context-frugal tool catalog** — 103 MCP tools grouped into 8 domains, loaded lazily. Startup surface is **~3,540 tokens** (~1.8% of a 200K window) vs ~24,918 if every tool registered eagerly. Agent expands a domain when it needs the rest. See [§ 103 tools, domain-gated](#-103-tools-but-they-dont-all-crowd-your-context--domain-gating-keeps-the-catalog-small).
...
| 🤖 **[Agent rules + all 103 tools](docs/AGENT_INSTRUCTIONS.md)** | 🛡️ **[Compliance & assurance](docs/COMPLIANCE.md)** (FISMA, CMMC, GDPR) |
...
| | |
|---|---|
| **103 MCP tools** | Memory, search, GDPR, refresh lifecycle — plus agent registry, handoffs, notifications, tasks, entity graph, fact enrichment, chat-log capture, and a 26-tool files-memory layer (directory ingestion, hierarchical chunking, ascension to core memory, watch-mode staleness review) |
