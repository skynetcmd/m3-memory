...
> An earlier oracle-routed configuration scored 89.0% (with oracle category metadata).


This report evaluates the production **no-oracle** LongMemEval-S configuration for m3-memory. Unlike the earlier oracle-routed configuration, which used oracle dataset `question_type` labels for routing, this v3 configuration infers all routing signals from the question text at runtime and does not use privileged dataset metadata.
...
**Retrieval accuracy — m3's core strength.** On the binary per-question `recall_any@k` convention (the "R@k" the adjacent LongMemEval submissions report), the v3 core engine reaches **99.2% session-hit-rate @ k=10 and 100% @ k=20** — raw turns, hybrid FTS5 + BGE-M3 vector + MMR, no knowledge graph, no oracle metadata. The right evidence session is in the top-10 for >99% of questions and in the top-20 for **all 500**. This is state-of-the-art for a fully local-first substrate and is the metric that isolates what the memory layer actually does. (Per-question-type SHR is in the [Retrieval](#retrieval) section. Its overall row is computed under a slightly different per-type aggregation and therefore reads marginally higher at the shallower depths — 98.8% vs 98.2% at k=5 and 99.4% vs 99.2% at k=10; k=20 and k=50 are 100.0% under both. The headline figures quoted here and in the README are the more conservative binary per-question values throughout.)

**End-to-end QA accuracy — strong, but answer-model-dependent.** With **no oracle metadata** (routing inferred from the question text at runtime), the full v3 configuration scores **92.0% (460/500)** using a frontier answerer (Opus 4.6) and the unmodified upstream gpt-4o judge. Because this number rides on the reader LLM, it should only be compared against *other systems' QA-accuracy figures*, never against their retrieval/recall numbers — **and only against figures graded by the same judge.** We grade with the verbatim upstream LongMemEval judge (six per-question-type templates, direct yes/no, no chain-of-thought, no leniency bias). On a 2026-06-23 audit of competitor judge prompts, agentmemory and Mastra OM also use the exact upstream judge (so their QA numbers are comparable to ours), whereas Mem0 and Memento use self-authored, more-lenient judges (pro-yes bias / semantic-equivalence leniency / chain-of-thought), which inflate scores and make a direct comparison to our 92.0% invalid; Chronos and Hindsight do not publish their LongMemEval judge.
...
> **Note on the Overall row.** These per-type figures aggregate differently from the binary per-question `recall_any@k` convention quoted in [Two different metrics](#two-different-metrics--retrieval-accuracy-vs-end-to-end-qa-accuracy) and in the README, so the Overall row reads marginally higher at shallow depth: **98.8% vs 98.2%** at k=5 and **99.4% vs 99.2%** at k=10. At k=20 and k=50 both conventions give 100.0%. Headline claims use the lower binary values.
